...
{
    public sealed class GetAllGlobalAssetsUrlsQueryHandler(
        IFileServiceReadOnlyClient fileServiceClient, 
        IGlobalAssetClient assetClient) : IRequestHandler<GetAllGlobalAssetsUrlsQuery, Result<List<AssetUrlResponse>>>
    {
        public async Task<Result<List<AssetUrlResponse>>> Handle(GetAllGlobalAssetsUrlsQuery request, CancellationToken cancellationToken)
        {
            Result<List<AssetMetadataResponse>> assetsMetadataResponse = await assetClient.GetFilesMetadata();
